...
# - Chapter 3
# - Positive & negative slopes are ran together here. It seems that doing so is ok. See Getis & Ord 1995
...
# Here I am running negative and positive slopes together.
 ...
 foreach (k= 1:length(rslopes)) %do% {
  ...
  upDist <- c(3500) # distance bands, 3km is a distance that works for the 4 periods using MOdis sinu proj. But 3.5 km is needed when using OMN projection.
  ...
  foreach (j= 1:length(upDist)) %do% {
    ...
    #wt1 <- st_weights(nb1, style = "W", allow_zero = NULL) # It seems Gi does not require a weight obj
    
    Gi <- sfdep::local_gstar_perm(slopePts$slope3, nb1, wt1, nsim = 999, alternative = "two.sided")
    Gi$id <- 1:nrow(Gi)
    
    GiSpDf <- slopePts %>% inner_join(Gi)
    
    # write files as geopackage as shapefiles truncate names
    sf::write_sf(GiSpDf, paste0(outf4, periodLabs[k], "/", prefix1, "gi_grbr5", "_" ,upDistLabs[j], "_pj_" , frq1, ".gpkg"), overwrite=TRUE)
...
# Note
# - gi_grbr4 files use distance band=3000km. this works when using MODIS proj. Note this don't use weights.
# - gi_grbr5 files use distance band=3500km. this works when using OMNR proj. Note this don't use weights.

# to add above
GiSpDf$cluster2[GiSpDf$"gi_star"  > 0  &  GiSpDf$"p_folded_sim"  <= 0.01] <- "very hot"
GiSpDf$cluster2[GiSpDf$"gi_star"  > 0  & GiSpDf$"p_folded_sim" > 0.01 & GiSpDf$"p_folded_sim" <= 0.05] <- "hot"
GiSpDf$cluster2[GiSpDf$"p_folded_sim"  > 0.05 ] <- "non-sig"
GiSpDf$cluster2[GiSpDf$"gi_star"  < 0  & GiSpDf$"p_folded_sim" > 0.01 & GiSpDf$"p_folded_sim" <= 0.05] <- "cold"
GiSpDf$cluster2[GiSpDf$"gi_star"  < 0  &  GiSpDf$"p_folded_sim"  <= 0.01] <- "very cold"
